...
struct node *delete_beginning(struct node *head)
{
    ...
    head = head->next; // making new head as head->next; i.e. what is next to head
    free(ptr); // freeing the memory of ptr, or previous head
...
int main()
{
    ...
    head = delete_beginning(head); // passing head to delete it and updating head to next pointer or linked node
